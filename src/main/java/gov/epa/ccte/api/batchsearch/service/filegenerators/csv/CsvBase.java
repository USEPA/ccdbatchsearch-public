...
@Slf4j
abstract class CsvBase extends ExportBase {

    private StringWriter sw;
    private CsvWriter csv;
    private List<String> headers;
    private List<String> headerWithToxprint;

    // for chemical lists export with csv
    ...
    protected void addHeader(String[] mandatory, String[] selected, String[] selectedChemicalLists) {
...
//        String[] combine = Stream.of(mandatory, selected)
//                .flatMap(Stream::of).toArray(String[]::new);

        List<String> toxprintHeader = Arrays.asList(toxprintHeaderValues.split(","));

        List<String> combine = Stream.of(mandatory, selected)
        ...
        // Check if combine contains MS_READY_SMILES and add two headers MS_READY_MASS,MS_READY_FORMULAE
        if (combine.contains("MS_READY_SMILES")) {

            combine.addAll(Arrays.asList("MS_READY_MASS","MS_READY_FORMULAE"));
        }

        // check for Toxprint - in toxprint case,
        // we need to remove it from the list and append it with toxprint columns
        if(combine.contains("TOXPRINTS_CHEMOTYPER")){
            // remove from header and add actual finger print headers added to csv file
            combine.remove("TOXPRINTS_CHEMOTYPER");

           List<String> headerWithToxprint = new ArrayList<>(combine);
           headerWithToxprint.addAll(toxprintHeader);

           csv.writeRow(headerWithToxprint);

           this.headerWithToxprint = headerWithToxprint;

           combine.add("TOXPRINTS_CHEMOTYPER"); // adding at the for helping with filling values

        }else{
            this.headerWithToxprint = null;
            csv.writeRow(combine);
        }

        this.headers = combine; // later use for data rows - toxprint should be the last header
    }
    ...
    protected void addNotFoundRow(String searchWord) {
        ...
        row[columnIndex++] = searchWord;

        row[columnIndex++] = Validators.getDataNotFoundMsg(searchWord);

        for (int i = columnIndex; i < headers.size(); i++)
            row[i] = EMPTY_VALUE;

        csv.writeRow(row);
    }

    protected void addDataRow(String searchWord, SearchWithChemicalDetails details, String[] selectedChemicalLists) {

        ArrayList<String> row = new ArrayList<>();

        for (String column : headers) {
            if(column.equalsIgnoreCase("TOXPRINTS_CHEMOTYPER")) {
                String toxPrint = getColumnContents(searchWord, column, details);
                for(String c : toxPrint.split("\t")){
                    row.add(c);
                }
            }else {
                row.add(getColumnContents(searchWord, column, details));
            }
            String cellValue = getColumnContents(searchWord, column, details);

            if(cellValue.equalsIgnoreCase(column)){
                int rowIndex= row.indexOf(column);
                if(chemlistLookup != null && chemlistLookup.contains(details.getDtxsid() + "-" + column))
                    row.set(rowIndex,"Y") ;
                else
                    row.set(rowIndex,"-") ;
            }
    ...
    protected String processToxprintForCsv(String value){
    ...
    protected byte[] getContents() {
        log.debug("returning data in byte[]");
        return sw.toString().getBytes();
    }

    public MediaType getCsvContentType() {
        log.debug("media type = {}", "text/csv");
        return MediaType.parseMediaType("text/csv");
    }

    public String getCsvFilename() {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd_hh_mm_ss");
        String excelFileName = "CCD-Batch-Search_" + formatter.format(LocalDateTime.now()) + ".csv";
